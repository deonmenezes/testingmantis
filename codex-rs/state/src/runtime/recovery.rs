//! Backup-and-rebuild support for Mantis runtime SQLite databases.
//!
//! Mantis keeps several independent runtime SQLite databases under one SQLite
...
/// Move one Mantis runtime SQLite database out of the way so that database can
    ...
    match tokio::fs::metadata(sqlite_home).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            ...
            Err(std::io::Error::other(format!(
                "no Mantis runtime database files were found to back up for {}",
                db_path.display()
            )))
        }
        Err(err) => Err(err),
...
async fn backup_sqlite_paths(
    ...
    if backups.is_empty() {
        ...
        return Err(std::io::Error::other(
            "no Mantis runtime database files were found to back up",
        ));
    }
