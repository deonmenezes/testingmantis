...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Mantis sandbox."
        );
        return;
    }
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Mantis to apply proposed code changes?".to_string());
...
/// Create a Mantis config that uses the mock server as the model provider.
