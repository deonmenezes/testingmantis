...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    metrics.gauge_with_description(
        "codex.active",
        "Number of active Mantis operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    assert_eq!(counter.description(), "Total number of Mantis turns.");
    ...
    assert_eq!(gauge.description(), "Number of active Mantis operations.");
